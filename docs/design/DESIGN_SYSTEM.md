...
| ------------------------------------------------------------------------ | ---------------------------------------------- | ---------------------------------------------- | ----------------------------------------------------------- |
| `--bg`                                                                   | oklch(0.975 0.003 250)                         | oklch(0.17 0.008 265)                          | Canvas                                                      |
| `--surface` / `--surface-2`                                              | 0.995 / 0.955                                  | 0.215 / 0.26                                   | Cards / raised surfaces                                     |
| `--border` / `--border-strong`                                           | 0.9 / 0.78                                     | 0.32 / 0.44                                    | Hairlines / emphasis                                        |
| `--ink` / `--ink-muted`                                                  | oklch(0.185 0.012 265) / 0.45                  | oklch(0.955 0.005 265) / 0.73                  | Text and **all numbers** (≥7:1 / ≥4.5:1)                    |
...
| `--pop-coral` / `-soft` / `--on-pop-coral`                               | oklch(0.62 0.19 30)                            | oklch(0.7 0.17 30)                             | Quartet — categorical only                                  |
| `--pop-teal` / `-soft` / `--on-pop-teal`                                 | oklch(0.55 0.11 175)                           | oklch(0.72 0.11 175)                           | Quartet                                                     |
| `--pop-amber` / `-soft` / `--on-pop-amber`                               | oklch(0.72 0.14 80)                            | oklch(0.8 0.14 82)                             | Quartet                                                     |
| `--pop-blue` / `-soft` / `--on-pop-blue`                                 | oklch(0.5 0.16 255)                            | oklch(0.72 0.13 250)                           | Quartet                                                     |
| `--hero` / `--on-hero` / `--hero-muted` / `--hero-line`                  | oklch(0.21 0.018 265) / 0.975 / 0.72 / 0.34    | oklch(0.145 0.014 265) / 0.965 / 0.71 / 0.3    | The wallet card — dark in both themes                       |
...
| `--chart-line` / `--chart-fill` / `--chart-grid`                         | accent / accent α.08 / 0.916                   | accent / accent α.12 / 0.27                    | Charts on the canvas                                        |
| `--glass-bg` / `--glass-bg-solid`                                        | oklch(0.985 0.002 250 / 0.66) / 0.975          | oklch(0.21 0.012 265 / 0.62) / 0.215           | Glass fill / **no-filter fallback and contrast worst case** |
| `--glass-border` / `--glass-highlight` / `--glass-shadow`                | ink α.12 / white α.6 / 0 8px 32px              | white α.08 / white α.09 / 0 8px 32px           | Glass edge, inset top highlight, drop                       |
| `--shadow-1` / `--shadow-2` / `--shadow-hero`                            | ink-tinted α.06 / α.10 / α.25                  | black α.4 / α.5 / α.5                          | Card / raised / hero elevations                             |

Live-mode overlay (`[data-mode="live"]`): `--mode-texture-opacity: 0`; `--hero` oklch(0.13 0 0) (dark theme oklch(0.1 0 0)); `--hero-line` and `--chart-grid-on-hero` go chroma-zero. Everything else is unchanged — the mode never restyles the canvas.
