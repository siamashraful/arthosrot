...
Paper trading is not real trading, even with a realistic execution venue. Ledgerline aims to be realistic without making false claims:
...
- **Sandbox infrastructure** may behave differently from production brokerage systems (latency, occasional resets); sandbox state is treated as disposable — Ledgerline's own ledger and canonical event history are the durable record.
