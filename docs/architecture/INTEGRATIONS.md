...
- **Limitations / risks:** contractually a development/testing environment — an indefinitely public prototype is a gray zone; sandbox data may be purged; simulated liquidity (see ../LIMITATIONS.md). Sandbox state is treated as **disposable** — Ledgerline's ledger + canonical events are the durable record.
